...
if($is_admin) {
    $store_r = sql_query("select * from `g5_write_main` where wr_is_comment=0 ");
    while ($row = sql_fetch_array($store_r)) {
        $list[] = $row;
    }
//등록대기 상점
    $count = sql_query("select * from `store_temp` where status = 0 ");
                        ...
                        <li>
                            <div class="section01_header store_title">
                                ...
                                <div class="store_edit">
                                    <a href="<?php echo G5_URL;?>/admin/my_store_detail_form.php?w=cu&wr_id=<?php echo $list[$i]["wr_id"]?>"><i></i></a>
                                </div>
                                <div class="store_del">
                            ...
                            <div class="section01_content">
                                <div style="text-align: center;background:#ddd;text-align: center;display: block;">
                                    <a href="<?php echo G5_URL;?>/admin/my_store_view.php?wr_id=<?php echo $list[$i]["wr_id"];?>&type=shop">
                                        <?php if($img_content) {echo $img_content;}else{?>
                                            <img src="<?php echo G5_IMG_URL?>/no-image.jpg" alt="no-image" style="text-align: center">
                                        <?php }?>
                                    </a>
                                </div>
                            </div>
                            <div class="list_btn_group">
                                <input type="button" value="메뉴수정" class="btn bg_darkred grid_50 list_menu_edit_btn" onclick="location.href='<?php echo G5_URL."/admin/my_store_menu_edit.php?wr_id=".$list[$i]["wr_id"]?>'">
                                <input type="button" value="<?php if($list[$i]["wr_5"]=="N"){?>상점오픈<?php }else{?>상점닫기<?php }?>" class="btn <?php if($list[$i]["wr_5"]=="N"){?>bg_gray<?php }else{?>bg_green<?php }?> grid_50 list_menu_edit_btn" onclick="location.href='<?php echo G5_URL."/admin/store_open_update.php?wr_id=".$list[$i]["wr_id"]."&state=".$list[$i]["wr_5"];?>'">
                            </div>
                        </li>

                    <?php 
						 $img_content = "";
					} if(count($list)==0 && count($temp)!=0){?>
                        <li>

...
if($_FILES["slide_file"]["name"]!=""){
	...
	$tmp_file  = $_FILES['slide_file']['tmp_name'];

    if ($error = is_uploaded_file($tmp_file)) {
        ...
        // 올라간 파일의 퍼미션을 변경합니다.
        chmod($dest_file, G5_FILE_PERMISSION);
        $sql = "insert into `store_slide` (wr_id,file_name) values ('{$wr_id}','{$filename}')";
        sql_query($sql);
    }else{
        alert("파일용량 확인 바랍니다.");
    }
...
if($t=="add"){    
    $key=sql_fetch("select * from `g5_write_main` where mb_id ='{$mb_id}' order by `wr_id` asc");
    sql_query("insert into `store_detail` (`wr_id`) values('{$key['wr_id']}');");
}
...
if ($file_upload_msg)
	...
	goto_url(G5_URL.'/admin/my_store_detail_form.php?page='.$page.'&t=up&wr_id='.$wr_id);
else if($bo_table=="inquiry")
	goto_url(G5_URL.'/page/inquiry/index.php?'.$qstr);
else
    goto_url(G5_URL.'/page/shop/my_store_detail_form.php?wr_id='.$wr_id.$qstr);
}else{
	goto_url(G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&page='.$page.'&wr_id='.$wr_id);
}
?>

...
if (!$mb['mb_id'] || !check_password($mb_password, $mb['mb_password'])) {
    $sql = "select count(*) as cnt from `g5_social_member` where mb_id = '{$mb_id}'";
    $chkmb = sql_fetch($sql);
    if($chkmb[cnt]>0){
        alert("소셜정보로 가입한 회원입니다.\\n소셜정보로 가입한 회원의 경우 가입한 소셜아이디로 로그인 후 \\n정보수정 메뉴를 통해 비밀번호를 바꿔야지만 일반 로그인이 가능합니다.");
    }else {
...
/*
if($lat&&$lng){
    goto_url(G5_URL."/page/matzip.php?lat=".$lat."&lng=".$lng);
}else{
	goto_url($link);
}*/
?>

...
if($lat && $lng){
    $query=sql_query("select a.*, b.* , 6371 * 2 * ATAN2(SQRT(POW(SIN(RADIANS({$lat} - b.latitude)/2), 2) + POW(SIN(RADIANS({$lng} - b.longitude)/2), 2) * COS(RADIANS(b.latitude)) * COS(RADIANS({$lat}))), SQRT(1 - POW(SIN(RADIANS({$lat} - b.latitude)/2), 2) + POW(SIN(RADIANS({$lng} - b.longitude)/2), 2) * COS(RADIANS(b.latitude)) * COS(RADIANS({$lat})))) AS distance from g5_write_main as a left join store_detail as b on a.wr_id = b.wr_id where a.mb_id='admin' and a.wr_is_comment=0 and a.wr_file != 0 and a.wr_6 = 'Y' {$where} order by a.wr_1 asc, a.wr_5 desc");
}else{
    $query=sql_query("select a.*, b.* from g5_write_main as a left join store_detail as b on a.wr_id = b.wr_id where a.mb_id='admin' and a.wr_is_comment=0 and a.wr_file != 0 and a.wr_6 = 'Y' and mb_id != 'admin' {$where} order by a.wr_1 asc, a.wr_5 desc");
}

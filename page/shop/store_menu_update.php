...
//    sql_query($sql);
	...
	$menu_price_discount = $_REQUEST["menu_price_discount"];
    ...
    $sql = "insert into `store_menu` (`wr_id`,`ca_name`,`menu_name`,`menu_detail`,`menu_price`,`menu_image`,`option`,`option_price`, `menu_price_discount`) VALUES  ('{$wr_id}','{$cate_name}','{$menu_name}','{$menu_detail}','{$menu_price}','{$filename1}','{$option}','{$option_price}' , '{$menu_price_discount}')";
    sql_query($sql);
//    alert("등록 되었습니다.",G5_URL."/page/shop/my_store_menu_2depth_edit.php?wr_id={$wr_id}&ca_name={$cate_name}");
    ...
    $sql = "update `store_menu` set menu_name = '{$menu_name}', menu_detail = '{$menu_detail}' , menu_price = '{$menu_price}', menu_price_discount = '{$menu_price_discount}', menu_image = '{$menu_image}', `option` = '{$option}' , `option_price` = '{$option_price}' {$photo_sql} where id ='{$menu_id}'";
    sql_query($sql);
